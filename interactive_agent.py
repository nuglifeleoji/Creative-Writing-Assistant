...
### 书本选择策略：
1. **首次使用**：如果用户没有指定书本，先调用 list_available_books_tool 查看可用书本
2. **自动选择**：如果只有一本书，自动选择该书
3. **用户选择**：如果有多本书，询问用户选择哪本书
4. **智能推荐**：根据用户问题内容，推荐最相关的书本
...
### 使用建议：
1. **书本管理优先**：确保在进行分析前已选择正确的书本
2. **展示RAG过程**：先调用 *_retrieve_tool 展示检索到的内容，再调用 *_generate_tool 进行LLM生成
3. **独立LLM调用**：当需要创造性内容或复杂分析时，使用 llm_generate_tool 或 llm_analyze_tool
4. **完整流程**：或者直接使用原有的完整工具（如 global_search_tool）
...
from datetime import datetime
from langchain.chains import LLMChain
from langchain.prompts import PromptTemplate  # 使用 PromptTemplate 替代 ChatPromptTemplate
from langchain_openai import AzureChatOpenAI

class DuneQuestionerAgent:
    def __init__(self):
        """初始化专注于《沙丘》小说的提问者Agent"""
        self.llm = self._init_azure_llm()
        self.chain = self._build_question_chain()
    ...
    def _build_question_chain(self) -> LLMChain:
        """构建针对《沙丘》的问题生成链"""
        # 详细的《沙丘》主题提示模板
        template = """
        # 角色：科幻小说专家，专注于《沙丘》系列
        ...
        # 要求：
        # 1. 问题类型分布：
        #    - 15% 人物关系（如厄崔迪家族与哈克南家族的恩怨）
        #    - 15% 世界观与设定（如沙漠星球阿拉基斯、香料美琅脂、弗瑞曼人）
        #    - 20% 重要故事情节（如保罗·厄崔迪的崛起、水之仪式）
        #    - 15% 冲突、悬念与伏笔（如预言、政治阴谋、生态转变）
        #    - 10% 人物情感曲线（如杰西卡夫人的内心挣扎、保罗的预知能力负担）
        #    - 25% 二次创作（如"如果保罗没有成为穆阿迪布会怎样？"）
        # 2. 避免重复历史问题：{previous_questions}
        # 3. 每次生成{num_questions}个问题
        # 4. 输出格式：JSON数组，例如：["问题1", "问题2", ...]
        # 5. 问题应深入小说细节，展现对原著的理解深度
        
        历史问题：{previous_questions}
        需要生成的问题数量：{num_questions}
        """
        
        # 使用 PromptTemplate 替代 ChatPromptTemplate
        prompt = PromptTemplate(
            template=template,
            input_variables=["previous_questions", "num_questions"]
        )
    ...
    def generate_questions(
        ...
        num_questions: int = 5  # 默认生成5个问题
    ) -> List[str]:
        """
        生成新的《沙丘》相关问题列表
        :param previous_questions: 避免重复的历史问题列表
        :param num_questions: 生成问题的数量
        :return: 新问题列表
        """
        try:
            # 准备输入参数
            input_data = {
                "previous_questions": "\n".join(previous_questions[-10:]) if previous_questions else "无",
                "num_questions": num_questions
            }
            
            # 调用LLM生成问题
            response = self.chain.invoke(input_data)
            text_output = response['text'] if isinstance(response, dict) else response
    ...
    def _parse_response(self, response: str) -> List[str]:
        ...
        for line in response.split("\n"):
            ...
            elif '?' in stripped:  # 包含问号的行
                questions.append(stripped)
        
        return questions[:10]  # 最多返回10个问题
    
    def _fallback_questions(self, num_questions: int) -> List[str]:
        """《沙丘》主题后备问题"""
        base_questions = [
            # 人物关系
            "保罗·厄崔迪和菲德-罗萨·哈克南之间有哪些关键冲突？",
            "杰西卡夫人与贝尼·杰瑟里特姐妹会的关系如何影响故事发展？",
            
            # 世界观与设定
            "香料美琅脂在《沙丘》宇宙中的经济和政治意义是什么？",
            "弗瑞曼人的文化和生存策略如何适应阿拉基斯的沙漠环境？",
            
            # 故事情节
            "描述保罗·厄崔迪在沙漠中生存并成为弗瑞曼人领袖的关键事件",
            "水在阿拉基斯星球和弗瑞曼文化中的象征意义是什么？",
            
            # 冲突悬念
            "贝尼·杰瑟里特姐妹会的育种计划如何成为整个故事的伏笔？",
            "预言中的'利桑·阿尔-盖布'如何影响保罗的行动和内心冲突？",
            
            # 人物情感
            "保罗在预见未来时的情感挣扎如何体现他的人物成长曲线？",
            "杰西卡夫人对公爵的爱与对姐妹会的责任之间的内心冲突",
            
            # 二次创作
            "如果哈克南家族没有背叛厄崔迪家族，故事会如何发展？",
            "设想一个平行宇宙，香料美琅脂被发现在一个水资源丰富的星球"
        ]
        
        return base_questions[:num_questions]

class InteractionController:
    def __init__(self, answerer_agent):
        self.questioner = DuneQuestionerAgent()
        self.answerer = answerer_agent
        self.data_store = []
        self.question_history = []
    
    def run_interaction_cycle(self, cycles=10):
        for _ in range(cycles):
            # 生成新问题 - 移除 domain 参数
            new_questions = self.questioner.generate_questions(
                previous_questions=self.question_history,
                num_questions=1  # 每次生成1个问题
            )
            
            for question in new_questions:
                # 获取回答
                answer = self.answerer.answer(question)  # 假设你的回答者有此方法
                
                # 存储数据
                self._store_data(question, answer)
                self.question_history.append(question)
                
                print(f"Q: {question}\nA: {answer}\n{'='*50}")
    
    def _store_data(self, question, answer):
        record = {
    ...
    def save_data(self, filename="finetuning_data.jsonl"):
        with open(filename, 'a', encoding='utf-8') as f:
            for record in self.data_store:
                f.write(json.dumps(record, ensure_ascii=False) + '\n')
class AnswererAdapter:
    """适配器类，为 GraphAnalysisAgent 添加 answer() 方法"""
    ...
    def answer(self, question: str) -> str:
        """回答关于《沙丘》小说的问题"""
        try:
            print(f"🤖 正在回答关于《沙丘》的问题: {question[:50]}...")
            
            # 使用 GraphAnalysisAgent 的全局搜索功能获取答案
            response = asyncio.run(self.graph_agent.global_search_full_async(question))
    ...
    def _generate_answer(self, question: str) -> str:
        ...
        try:
            # 构建提示
            prompt = f"""
你是一个《沙丘》小说专家，请基于小说内容详细回答以下问题：
问题：{question}

回答要求：
1. 提供详细、具体的解释
2. 包含小说中的具体细节和证据
3. 如果涉及人物，说明他们的角色、特点和重要性
4. 如果涉及事件，描述其背景、过程和影响
5. 回答长度应在200-500字之间
"""
            ...
            from langchain_core.messages import SystemMessage, HumanMessage
            messages = [
                SystemMessage(content="你是一个专业的《沙丘》小说分析助手，请基于小说内容详细回答问题。"),
                HumanMessage(content=prompt)
            ]
            ...
            print(f"✅ LLM生成回答完成，长度: {len(result_text)} 字符")
            return result_text
            
        except Exception as e:
            print(f"❌ LLM生成回答失败: {str(e)}")
            return f"无法回答此问题: {str(e)}"

if __name__ == "__main__":
    # 初始化 GraphAnalysisAgent
    graph_agent = GraphAnalysisAgent(use_multi_book=True)
    
    # 创建适配器
    answerer = AnswererAdapter(graph_agent)
    ...
    print("数据收集完成，已保存到 dune_finetuning_data.jsonl")
